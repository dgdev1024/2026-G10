...
#   and link together into one executable.
    ...
    if [[ -d "$entry" ]]; then
        ...
        for asm_file in "$entry"/*.s "$entry"/*.asm; do
            if [[ -f "$asm_file" ]]; then
                ...
                obj_file="$OBJ_OUTPUT_DIR/$(basename "${asm_file%.*}.g10obj")"
        ...
        # Hex dump the output executable file for inspection
        echo "Hex dump of linked executable: $exe_file"
        hexdump -C "$exe_file"
        echo ""
    elif [[ -f "$entry" ]]; then
