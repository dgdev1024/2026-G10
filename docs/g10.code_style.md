...
- `.inl` - Inline Function Definitions (if necessary)
...
- **Braces**: Use Allman style for braces:
    ...
    - For short blocks, braces are still required, but may be placed on the same
        ...
        if (condition)
            ...
            { /* code block */ }
        ```
    - Use Allman style for class/struct/enum/union definitions as well:
        ```cpp
...
- **Spaces**:
    ...
    - In template declarations, place a space after the opening angle bracket and
        ...
        template <typename T>
        ```
    - In class/struct constructor initializer lists, place the space after the
        closing parenthesis and before the colon, a new line after the colon, and 
        each initializer on its own line with a space after the comma. Use braces
        for member initializations and align them vertically:
        ```cpp
        my_class::my_class (int int_param, double double_param) :
            m_int_param     { int_param },
            m_double_param  { double_param }
        {
            // constructor body
        }
        ```
    - In class/struct/union member initializers, place a space after the name,
        after the opening brace, and before the closing brace:
        ```cpp
            std::int32_t m_the_answer { 42 };
        ```
- **Functions and Methods**:
    - Use the `auto ->` syntax for function and method return types:
        ```cpp
        auto my_function (int param) -> void;
        ```
    - Declarations and definitions should have the return type on the same line as
        the function name:
        ```cpp
        auto my_function (int param) -> void
        {
        ...
        ```
    - For functions with long parameter lists, break lines logically:
        ```cpp
        auto my_function (int param1, double param2, const std::string& param3,
            bool flag) -> void
        {
            // function body
        }
...
- **Code Organization**:
    - Use comments as section dividers to group constants/enums, unions/structs,
        ...
        namespace g10
        {
            ...
            // last
            //
            // Exception: If a certain constant (such as a lookup table) relies
            // on a specific enum, that enum should be defined first.
        }
        ...
        namespace g10
        {
            ...
            //
            // Exception: If a certain union relies on a specific struct, that
            // struct should be defined first.
        }
        ...
        namespace g10
        {
            ...
            private: /* Private Methods - Specific Grouping *******************/
                ...
                // `Public Methods`, `Protected Methods` and `Private Methods` 
                // can be further grouped into specific functionality sections 
                // using additional divider comments as needed.
    ...
    - Implementation files follow a similar structure and ordering as header
        ...
        namespace g10
        {
            // Same order and exceptions as in header files
        }
